...
def test_policy():
    ...
    # Test call
    action_probs = jnp.exp(policy.apply(
        params, next(rng), timesteps, actions, rewards))
...
def test_policy_rollout():
    ...
    policy = Policy(config=config)
    params = policy.init(next(rng), next(rng), jnp.ones((
        1, 2), dtype=jnp.int32), jnp.ones((1, 2), dtype=jnp.int32), jnp.ones((1, 2)))

    def policy_fn(key, timesteps, actions, rewards): return policy.apply(
        params, key, timesteps, actions, rewards)
    ...
    assert actions.shape == (n_sample, config.horizon) and rewards.shape == (
        n_sample, config.horizon) and log_policy_probs.shape == (n_sample, config.horizon, conf.num_actions)
...
def test_rollout():
    ...
    num_samples = 2
    horizon = 4
    def policy_fn(k, t, a, r): return jnp.array([jnp.log(0.01), jnp.log(
        0.01), jnp.log(0.98)]).reshape(1, -1).repeat(t.shape[0], axis=0)
    ...
    assert actions.shape == (num_samples, horizon) and \
        ...
        log_policy_probs.shape == (num_samples, horizon, num_actions)
